...
7. 借鉴 Office 的操作风格，如果以 shape 为边框线，那么在“行高”和“列宽”更改时，“单元格”内的控件自动调整。
8. 某带区“列宽”调整，其他带区同位置“列宽”会自动调整。
9. 代码实现快速画“表格”。
10. 它在一定程度上实现了所见即所得。
...
增加：对 **TQ开发框架** 的直接支持，允许忽略参数 tcConnectionString

**2023.08.23**

版本：β1.0.38

增加：可以基于 SQL Server/MySQL 来创建新的报表
...
增加：
1. 可以在快速菜单中打开基于远程数据库的报表（目前仅支持 SQL Server/MySQL，如需增加其他数据库，请联系作者）
2. 在 OLD DB 连接方式下，设计器可以使用 Unicode 编码数据

修改：
1. 试图调整带区对象时报错；
2. 在重复打开 frx 文件但不打开 dbf 数据源的重复操作情况下，报错；
3. 菜单 开始-数据源-DBC 漏掉了文件路径问题，现在以绝对路径打开文件
